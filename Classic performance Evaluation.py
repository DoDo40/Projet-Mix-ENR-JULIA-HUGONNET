# -*- coding: utf-8 -*-
"""
@Time    : 25/05/2023 21:11
@Author  : dorianhgn
@FileName: Classic performance Evaluation.py
@Software: PyCharm
"""
import pandas as pd
import numpy as np
...
# Ici créer une politique si j'ai le temps
...
while not time_step.is_last():
    i += 1
    action = np.append(np.random.uniform(-1, 1, 3), np.random.uniform(-1, 0))  # Fais une action random
    time_step = tf_env.step(np.array([action]))  # Appliquer l'action à l'environnement
...
print("Avec une police random, les GWh non fournies sur l'année 2022 avec les données de RTE sont de " +
